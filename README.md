...
.info-form input:focus, .info-form select:focus { outline:none; border-color:#667eea; }
.info-form .required-star { color:#dc3545; margin-left:2px; }
.info-form .field-hint { font-size:12px; color:#888; margin-top:-10px; margin-bottom:12px; }
.dimensions-grid {
    display:grid;
...
.quiz-footer .btn { flex:1; padding:12px 30px; }
...
#result .result-header h2 { font-size:22px; margin-bottom:5px; }
#result .result-header .result-user-info { font-size:13px; color:#666; line-height:1.5; }
...
/* 特定区块占满整行 */
#result .result-section:nth-of-type(1) { flex: 0 0 100%; }  /* 摘要 */
#result .result-section:nth-of-type(4) { flex: 0 0 100%; }  /* T12排行 */
#result .result-section:nth-of-type(13) { flex: 0 0 100%; } /* 个人发展建议 */
...
.score-name { width:90px; font-size:13px; color:#333; flex-shrink:0; }
.score-bar {
    flex:1;
    height:18px;
    background:#f0f0f0;
...
.score-value { width:36px; text-align:right; font-size:14px; font-weight:600; color:#2a5298; flex-shrink:0; }
.score-tag {
    font-size:11px;
    padding:2px 8px;
    border-radius:10px;
...
.t12-card .t12-name { font-size:12px; color:#333; font-weight:500; margin-bottom:2px; }
.t12-card .t12-score {
    font-size:20px;
    font-weight:700;
    background:linear-gradient(135deg,#667eea,#764ba2);
...
.t12-card .t12-desc { font-size:9px; color:#888; line-height:1.2; }
...
.result-actions .btn { flex:1; min-width:120px; text-align:center; font-size:14px; padding:10px 20px; }
.footer-note {
    font-size:11px;
    color:#999;
    margin-top:15px;
...
@media print {
    ...
    .dimension-card,.option-item,.result-header,.analysis-box,.t12-card { -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; }
    .score-fill { -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; }
    .btn,.result-actions,.quiz-footer { display:none !important; }
    .section { display:block !important; }
    #welcome { display:none !important; }
    #quiz { display:none !important; }
    #result { display:flex !important; }
    #result .result-section { page-break-inside: avoid; }
}
...
@media (max-width:640px) {
    ...
    #result .result-section { flex: 0 0 100%; }
    .t12-grid { grid-template-columns:repeat(3,1fr); }
    .score-name { width:70px; font-size:12px; }
    .result-actions .btn { min-width:80px; font-size:13px; padding:8px 15px; }
    .question-text { font-size:16px; }
    .option-content { font-size:14px; }
}
@media (max-width:480px) {
    .t12-grid { grid-template-columns:repeat(2,1fr); }
    .dimensions-grid { grid-template-columns:1fr 1fr; }
    .quiz-footer { flex-direction:column; }
    .quiz-footer .btn { flex:none; }
}
</style>
</head>
<body>
<div class="container">
    ...
    <div class="content">
        ...
        <div id="welcome" class="section active welcome-page">
            ...
            <div class="intro">
                <h3>📋 测评说明</h3>
                <p>本测评针对招聘场景设计，从<strong>职业素质、职业性格、情商EQ、智商IQ</strong>四大维度全面评估候选人潜力，为企业提供精准用人参考，同时帮助个人认知自身优势。</p>
                <p style="margin-top:10px;">⏱️ 预计用时：25-35分钟 | 📊 专业分析报告 | 📄 下载PDF结果</p>
            </div>
            ...
            <div class="info-form">
                ...
                <input type="text" id="userDept" placeholder="如：生产主管、运营经理等">
                ...
                <div class="field-hint">⚠️ 以上三项为必填</div>
            </div>
        ...
        <div id="result" class="section">
            ...
            <div class="result-section"><h3>📊 四维综合评估</h3><div id="overallScores"></div><div id="overallInterpretation" class="analysis-box" style="margin-top:8px;"></div></div>
            <div class="result-section"><h3>🎯 领导力风格画像</h3><div id="leadershipStyle" class="analysis-box"></div></div>
            <div class="result-section"><h3>🏅 T12职业优势排行</h3><p style="font-size:12px;color:#888;margin-bottom:6px;">分数范围：30~70分，反映该特质在您身上的相对强度</p><div id="t12Grid" class="t12-grid"></div></div>
            ...
            <div class="result-section"><h3>❤️ 情商(EQ)深度解析</h3><div id="eqScores"></div><div id="eqInterpretation" class="analysis-box" style="margin-top:8px;"></div></div>
...
(function() {
    ...
    function showSection(id) {
        ...
        // 特殊处理：如果激活的是result，需要额外添加active类让#result显示为flex
        if (id === 'result') {
    ...
    function renderScoreBar(name, score) {
        ...
        return '<div class="score-row"><div class="score-name">'+name+'</div><div class="score-bar"><div class="score-fill" style="width:'+score+'%"></div></div><div class="score-value">'+score+'</div><span class="score-tag '+level.class+'">'+level.text+'</span></div>';
    }
    ...
    function showResult() {
        ...
        summary += '<div class="recruit-highlight"><strong>💼 管理潜力评估：</strong> ' +
            (scores.modules['职业素质']>=80 ? '具备较强的管理能力，可胜任主管及以上岗位。' :
             scores.modules['职业素质']>=60 ? '具备一定的管理基础，可通过培养提升。' : '管理经验尚浅，建议从基层管理岗位开始锻炼。') +
            '</div>';
        ...
        document.getElementById('leadershipStyle').innerHTML = '<p>'+leadershipStyles[style]+'</p><p style="margin-top:6px;">基于您的 T12 特质组合，您更倾向于 <strong>'+style+'</strong> 领导风格。</p>';

        // ---- T12职业优势排行 ----
        var t12Html = '';
        sortedT12.forEach(function(item) {
            var name=item[0], score=item[1];
            t12Html += '<div class="t12-card"><div class="t12-name">'+name+'</div><div class="t12-score">'+score+'</div><div class="t12-desc">'+(t12Dimensions[name]?t12Dimensions[name].desc:'')+'</div></div>';
        });
        ...
        top3.forEach(function(item) {
            swHtml += '<p>• <strong>'+item[0]+'</strong>（'+item[1]+'分）—— '+(t12Dimensions[item[0]]?t12Dimensions[item[0]].desc:'')+'</p>';
        });
        swHtml += '<p style="margin-top:8px;"><strong>⚡ 关键发展领域（Bottom3）</strong></p>';
        ...
        sortedF4.forEach(function(item, idx) {
            var field=item[0], score=item[1];
            var level = getT12Level(score);
            f4Html += '<p><strong>'+(idx+1)+'. '+field+'（'+score+'分 - '+level.text+'）</strong><br><span style="font-size:12px;color:#666;">'+f4Fields[field]+'</span></p>';
        });
        f4Html += '<p style="margin-top:6px;"><strong>✨ 核心优势领域：'+topField+'</strong></p>';
        ...
        top3.forEach(function(item) {
            personalityHtml += '<p>• <strong>'+item[0]+'</strong>（'+item[1]+'分）<br><span style="font-size:12px;color:#666;">'+(t12Dimensions[item[0]]?t12Dimensions[item[0]].desc:'')+'</span></p>';
        });
        personalityHtml += '<p style="margin-top:8px;"><strong>⚖️ 待发展特质（Bottom3）</strong></p>';
        bottom3.forEach(function(item) {
            personalityHtml += '<p>• <strong>'+item[0]+'</strong>（'+item[1]+'分）<br><span style="font-size:12px;color:#666;">'+(t12Dimensions[item[0]]?t12Dimensions[item[0]].desc:'')+'</span></p>';
        });
        ...
        var eqInterp = '';
        if (eqScore >= 80) eqInterp = '您的情商能力非常突出，尤其在自我认知和人际管理方面表现出色。这使您能够在复杂人际环境中游刃有余，是卓越领导者的重要特质。';
        else if (eqScore >= 60) eqInterp = '您的情商处于良好水平，具备基本的情绪感知和关系处理能力。建议在共情能力和冲突管理方面进行刻意练习，可显著提升领导效能。';
        else eqInterp = '您的情商能力有待提升，建议从自我觉察开始，学习情绪调节技巧，并多参与团队协作活动，逐步增强人际敏感度。';
        ...
        fitHtml += '<p>您的匹配度：<strong>'+fitLevel+'</strong>（匹配项：'+ (matchDetails.length?matchDetails.join('、'):'无') +'）</p>';
        ...
        if (scores.modules['情商EQ'] < 60) {
            recruitSuggestions.push('• 候选人情商低于平均，建议在面试中增加情境模拟，考察其应对压力和人际交往的真实表现。');
        }
        ...
        document.getElementById('recruitSuggestions').innerHTML = recruitSuggestions.map(function(s){ return '<li>'+s+'</li>'; }).join('');

        // ---- 个人发展建议 ----
        var personalSuggestions = generatePersonalSuggestions(scores);
        document.getElementById('personalSuggestions').innerHTML = personalSuggestions.map(function(s){ return '<li>'+s+'</li>'; }).join('');

        showSection('result');
        window.resultData = { scores:scores, userInfo:userInfo, sortedT12:sortedT12, sortedF4:sortedF4, style:style };
    }
    ...
    function generateResultText() {
        ...
        if (!data) return '暂无数据';
        var scores = data.scores, userInfo = data.userInfo, sortedT12 = data.sortedT12, sortedF4 = data.sortedF4;
        var lines = [];
        lines.push('========================================');
        lines.push('  T12职业优势测评报告（招聘甄选版）');
        lines.push('  设计人：DAVY（抖音号：d190564680）');
        lines.push('========================================\n');
        lines.push('【候选人】'+userInfo.name+' | '+userInfo.dept+' | '+userInfo.industry+' | '+userInfo.date);
        lines.push('\n【四维综合】');
        ...
        sortedT12.forEach(function(item, idx) {
            lines.push((idx+1)+'. '+item[0]+'：'+item[1]+'分 - '+(t12Dimensions[item[0]]?t12Dimensions[item[0]].desc:''));
        });
        lines.push('\n【四大领域】');
